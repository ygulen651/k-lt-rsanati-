...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const all = readAll()
  const item = all.find((m) => m.id === id && (m.group || '') === GROUP)
  if (!item) {
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const all = readAll()
    const idx = all.findIndex((m) => m.id === id && (m.group || '') === GROUP)
    if (idx === -1) return NextResponse.json({ success: false, message: 'Kayıt bulunamadı' }, { status: 404 })

    const updated = { ...all[idx], ...body, id, group: GROUP }
    all[idx] = updated
    writeAll(all)
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const remaining = all.filter((m) => !(m.id === id && (m.group || '') === GROUP))
