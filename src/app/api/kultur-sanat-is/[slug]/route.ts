...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    await connectDB()
    const item = await KamuAr.findOne({ slug, category: 'kultur-sanat-is' })
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const updated = await KamuAr.findOneAndUpdate({ slug, category: 'kultur-sanat-is' }, { ...body, category: 'kultur-sanat-is' }, { new: true, runValidators: true })
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const { slug } = await params
    await connectDB()
    const deleted = await KamuAr.findOneAndDelete({ slug, category: 'kultur-sanat-is' })
